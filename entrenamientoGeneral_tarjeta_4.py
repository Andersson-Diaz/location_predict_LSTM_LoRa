...
def ejecutar_entrenamiento(r,t,data):
    ...
    f = open ('best_values_tarjeta4_location.txt','w')
    ...
    #print(modelo.summary())
    modelo.save('model_LSTM_tarjeta4.h5')
    import joblib
    joblib.dump(scaler, 'scaler_tarjeta4.save')                
    ...
    best_hps = tuner.get_best_hyperparameters(num_trials = 1)[0]
    print(best_hps.values)
    f = open ('best_values_tarjeta4_time.txt','w')
    f.write(str(best_hps.values))
    f.close()
    ...
    #Guardamos el modelo
    model_time.save('tiempo_entrenado_tarjeta4.h5')
    import joblib
    #guardamos el normalizador
    joblib.dump(sc, 'scaler_tiempo_tarjeta4.save')   
    fin = time.time()
...
def read_db():
    ...
    dataset= pd.read_sql("SELECT * FROM data_set_training WHERE dev_id = 'tarjeta4-esp32lora' order by id",myConnection)
