...
def ejecutar_entrenamiento(r,t,data):
    ...
    f = open ('best_values_tarjeta2_location.txt','w')
    ...
    best_hps = tuner.get_best_hyperparameters(num_trials = 1)[0]
    print(best_hps.values)
    f = open ('best_values_tarjeta2_time.txt','w')
    f.write(str(best_hps.values))
    f.close()
...
def read_db():
    ...
    dataset= pd.read_sql("SELECT * FROM data_set_training WHERE dev_id = 'tarjeta2-cubecell' order by id",myConnection)
