...
def ejecutar_prediccion_escenario_1():#Sin LoRa
    try:
        print('inicio de ejecucion de escenario 2')
        import MySQLdb
        import pandas as pd
        import numpy as np
        ...
        dataset= pd.read_sql("SELECT * FROM LoRaWAN_messages_calle_5 order by id DESC LIMIT 31",myConnection)
        ...
        x_test= np.column_stack((set_prediccion.iloc[1:len(set_prediccion),[4]],set_prediccion.iloc[1:len(set_prediccion),[5]],set_prediccion.iloc[1:len(set_prediccion),[8]],set_prediccion.iloc[1:len(set_prediccion),[11]]))
        ...
        for i in range(0, len(set_prediccion)-1):
            distance_validation.append(haversine(set_prediccion.iat[i+1,4],set_prediccion.iat[i+1,5],set_prediccion.iat[i,4],set_prediccion.iat[i,5]))
...
def ejecutar_prediccion_escenario2(idx):#Cuando hay conexión LoRa sin GPS
    try:
      print('inicio de ejecucuion de escenario 2')
      import MySQLdb
      import pandas as pd
      #datos para la conexion a la base de datos
      hostname = '82.180.175.58'
      ...
      set_prediccion.reset_index(inplace=True, drop=True)
      #x_test_latitud = set_prediccion.iloc[:,4:5]
      x_test= np.column_stack((set_prediccion.iloc[:,[4]],set_prediccion.iloc[:,[5]],set_prediccion.iloc[:,[8]],set_prediccion.iloc[:,[11]]))
      ...
      X_pred = x_test_normalized.copy()
      for i in range(window,len(X_pred)):
            xin = (np.column_stack((X_pred[i-window:i,0:2],x_test_normalized[i-window:i,2:3],x_test_normalized[i-window:i,3:4]))).reshape(1, window, 4)
            X_pred[i] = new_model.predict(xin)
      ...
      #adr = ("C/ Samaranch 42", )
      #cadena_SQL = "INSERT INTO Tabla_General (latitude) VALUES (%s)"
      #cadena_SQL = "UPDATE Tabla_General SET (latitude) VALUES(%s) WHERE id =1"
      cadena_SQL = "UPDATE Tabla_General SET latitude = %s, longitude = %s, type_record = %s  WHERE id =%s"

      #cadena_SQL= "INSERT INTO Tabla_General (latitude, longitude, accx)  VALUES (%s,%s,%s)"
      #f = calle_5_p.iloc[window:,0:1].values
      l = prediction[window,0]
      t = prediction[window,1]
      val=(l,t,2,idx)
      #val=(prediction[window,0].astype(var),prediction[window,1].astype(var),2)
        #print (cadena_SQL)
...
def monitor():
    ...
    # genera la lectura de la base de datos
    dataset2= pd.read_sql("SELECT * FROM LoRaWAN_messages_calle_5 order by id DESC LIMIT 1",myConnection)
    dataset2['id']=dataset2['id'].astype('float64')
    idx = dataset2.iloc[0,[0]].values.astype('float64')    
    ...
    #Ha tardado un dato en llegar mas de 13 segundos?
        ...
        ejecutar_prediccion_escenario_1()
    else:
