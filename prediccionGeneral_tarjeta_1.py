...
def ejecutar_prediccion_escenario_1(id_anterior):  # Sin LoRa
    try:
        ...
        dataset = pd.read_sql("SELECT * FROM Tabla_General WHERE dev_id = 'tarjeta1-cubecell' order by id DESC LIMIT 31", mynewConnection)
        ...
        if 0 in dataset.latitude.values or '' in dataset.latitude.values:
            ...
            try:
                import joblib
                scaler = joblib.load('scaler_tarjeta1.save')
                #new_model = keras.models.load_model('model_bidirectional.h5')
                new_model = keras.models.load_model('model_LSTM_tarjeta1.h5')
            except OSError:
            ...
            try:
                import joblib
                sc = joblib.load('scaler_tiempo_tarjeta1.save')
                model_last = keras.models.load_model('tiempo_entrenado_tarjeta1.h5')
            except:
            ...
            while(bol):                    
                ...
                time.sleep(1)
                mynewConnection = MySQLdb.connect(host=hostname, user=username, passwd=password, db=database)
                dataset = pd.read_sql("SELECT * from Tabla_General WHERE dev_id = 'tarjeta1-cubecell' order by id DESC LIMIT 1", mynewConnection)                
                ...
                while(diferencia_tiempo[index_actual].total_seconds()<=1 and index_actual<29):                   
                    ...
                    val = ('tarjeta1-cubecell',tim, lat, lon, lat, lon, tim, 1, acc, giro)
                    time.sleep(0.2)
                    dataset = pd.read_sql("SELECT * from Tabla_General WHERE dev_id = 'tarjeta1-cubecell' order by id DESC LIMIT 1", mynewConnection)
                    if (id_anterior!=dataset.iloc[0,0] or index_actual>=28): 
                        bol = False
...
def ejecutar_prediccion_escenario2(ultimo_id):  # Cuando hay conexión LoRa sin GPS
    try:
        ...
        dataset = pd.read_sql("SELECT * FROM Tabla_General WHERE dev_id = 'tarjeta1-cubecell' order by id DESC", mynewConnection)       
...
def monitor(dataset2):
    ...
    if (dataset2.iloc[0,column_lat]!= ''):
        ...
        iron = pd.read_csv('id_prediccion_tarjeta1.csv')
...
def read_db():
    ...
    # genera la lectura de la base de datos
    dataset= pd.read_sql("SELECT * FROM Tabla_General WHERE dev_id = 'tarjeta1-cubecell' order by id DESC LIMIT 1",myConnection)
    myConnection.close()    
